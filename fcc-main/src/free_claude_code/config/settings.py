...
class Settings(BaseModel):
    ...
    # -------------------- Service-scoped convenience fields --------------------
    # These are additional, non-invasive fields to make running multiple instances
    # on the same machine easier. They do not change existing behavior unless you
    # explicitly use the SERVICE_PORT env vars or these attributes.
    server_port: int = Field(default=8082, validation_alias="SERVER_PORT")
    claude_port: int = Field(default=8083, validation_alias="CLAUDE_PORT")
    nemotron_port: int = Field(default=8084, validation_alias="NEMOTRON_PORT")

    # Global force restart flag (optional). Launchers should prefer CLI --force first,
    # then SERVICE_FORCE env vars, then this attribute.
    force_restart: bool = Field(default=False, validation_alias="FORCE_RESTART")

    # -------------------- End service-scoped convenience fields --------------------
    ...
    # -------------------- Convenience helpers for launchers --------------------
    # These helpers make it easy for launchers to pick a host/port/force for a
    # named service without editing the large settings file elsewhere.

    def get_service_host(self, service: Literal["server", "claude", "nemotron"] = "server") -> str:
        """
        Resolve host for a named service.

        Priority:
        1. Environment variable <SERVICE>_HOST (e.g., SERVER_HOST, CLAUDE_HOST)
        2. This Settings.host attribute
        3. Fallback "0.0.0.0"
        """
        import os

        env = os.environ.get(f"{service.upper()}_HOST")
        if env is not None and env != "":
            return env
        return getattr(self, "host", "0.0.0.0")

    def get_service_port(self, service: Literal["server", "claude", "nemotron"] = "server") -> int:
        """
        Resolve port for a named service.

        Priority:
        1. Environment variable <SERVICE>_PORT (e.g., SERVER_PORT, CLAUDE_PORT)
        2. Service-specific field on Settings (server_port, claude_port, nemotron_port)
        3. Settings.port
        4. Fallback defaults (server:8082, claude:8083, nemotron:8084)
        """
        import os

        env = os.environ.get(f"{service.upper()}_PORT")
        if env:
            try:
                return int(env)
            except ValueError:
        ...
        service_field = {
            "server": getattr(self, "server_port", None),
            "claude": getattr(self, "claude_port", None),
            "nemotron": getattr(self, "nemotron_port", None),
        }.get(service)

        if service_field:
            return int(service_field)

        # fallback to generic port
        if getattr(self, "port", None):
            return int(self.port)

        fallback_map = {"server": 8082, "claude": 8083, "nemotron": 8084}
        return int(fallback_map.get(service, 8083))

    def service_force_restart(self, service: Literal["server", "claude", "nemotron"] = "claude") -> bool:
        """
        Determine whether the launcher should force restart this service.

        Priority:
        1. Environment variable <SERVICE>_FORCE (truthy values: "1","true","yes")
        2. This Settings.force_restart attribute
        3. Default False
        """
        import os

        env = os.environ.get(f"{service.upper()}_FORCE")
        if env is not None:
            return env.strip().lower() in ("1", "true", "yes", "y")
        return bool(getattr(self, "force_restart", False))

    def resolve_service_binding(self, service: Literal["server", "claude", "nemotron"] = "server"):
        """
        Convenience: return a tuple (host, port, force) for the named service.
        """
        return (self.get_service_host(service), self.get_service_port(service), self.service_force_restart(service))
